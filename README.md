...
Run a local dev page with optional API mocks:

```bash
pnpm dev
# open http://localhost:5173/  (Vite default)
# add ?mock=1 to enable mocked endpoints
```

- The dev page loads the same floating UI as the userscript.
- When `?mock=1` is present, `window.fetch` is intercepted to return paginated mock data for `/users`, `/projects`, `/projects/{id}/executions`, `/executions/{id}/tasks`.
- A demo token is stored automatically for convenience.
